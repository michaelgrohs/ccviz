...
const WelcomePage: React.FC = () => {
    ...
    if (target.files && target.files[0]) {
      ...
      reader.onload = async () => {
        ...
        if (file.name.endsWith('.bpmn')) {
          ...
          const resp = await fetch('http://127.0.0.1:5000/api/bpmn-activities', {
            method: 'POST',
            body: formData,
          });
  ...
  const handleUploadOrNavigate = async () => {
    ...
    try {
      ...
      const attributeResponse = await fetch('http://127.0.0.1:5000/api/conformance-by-event_attribute');
      ...
      try {
        await fetch('http://127.0.0.1:5000/api/save-timing', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
